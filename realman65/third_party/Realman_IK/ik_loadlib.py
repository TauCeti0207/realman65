...
Author: tauceti0207
version: 
Date: 2025-11-02 16:46:01
LastEditors: tauceti0207
LastEditTime: 2025-11-03 14:03:38
'''
#! /usr/bin/env python3
#! -*-conding=: UTF-8 -*-
#
...
if os.name == 'nt':
    ...
    if os.name == 'posix':
        if 'DARWIN' in os.uname():
            ...
            if not os.path.exists(lib_name):
                raise Exception(
                    f"Error: {lib_name} does not exist.".format(lib_name))
        else:
...
func = lib.ikine
func.argtypes = [ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.POINTER(ctypes.c_float * 8), ctypes.POINTER(
    ctypes.c_float * 8), ctypes.POINTER(ctypes.c_float * 16), ctypes.POINTER(ctypes.c_float * 7), ctypes.POINTER(ctypes.c_float * 7)]
...
def lib_ikine(rbt_type, sr_type, q_ref: list, Ttarget: list, q_max, q_min, is_joint_space=False):
    if rbt_type == RobotType.RM75 or rbt_type == RobotType.GEN72:
        q_in = (ctypes.c_float * 7)(q_ref[0], q_ref[1],
                                    q_ref[2], q_ref[3], q_ref[4], q_ref[5], q_ref[6])
        q_max_c = (ctypes.c_float * 8)(q_max[0], q_max[1],
                                       q_max[2], q_max[3], q_max[4], q_max[5], q_max[6], 0)
        q_min_c = (ctypes.c_float * 8)(q_min[0], q_min[1],
                                       q_min[2], q_min[3], q_min[4], q_min[5], q_min[6], 0)
    else:
        q_in = (ctypes.c_float * 7)(q_ref[0], q_ref[1],
                                    q_ref[2], q_ref[3], q_ref[4], q_ref[5], 0)
        q_max_c = (ctypes.c_float * 8)(q_max[0], q_max[1],
                                       q_max[2], q_max[3], q_max[4], q_max[5], 0, 0)
        q_min_c = (ctypes.c_float * 8)(q_min[0], q_min[1],
                                       q_min[2], q_min[3], q_min[4], q_min[5], 0, 0)
    ...
    ret = func(rbt_type, sr_type, is_joint_space,
               q_max_c, q_min_c, T, q_in, q_out)
    return list(q_out), ret


if __name__ == "__main__":
    q_in = [1.04719755119660, 0.757472895365539, -
            1.75056523975031, 0, -0.523598775598299, 0]
    T = [[-0.0270394064923877, 0.866025403784439, 0.499268335163106, 0.0719079041256672],
         [-0.0468336258513231, -0.500000000000000,
             0.864758123112826, 0.124548143411447],
         [0.998536670326212, 5.55111512312578e-17,
             0.0540788129847752, 0.548971876651408],
         [0, 0, 0, 1]]
