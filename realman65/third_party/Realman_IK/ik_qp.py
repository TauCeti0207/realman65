...
def RM65_Demo():
    ...
    q_ref = np.array([0, 25, 90, 0, 65, 0]) * deg2rad  # 输入：起始关节角
    Td = robot.fkine(q_ref)         # 目标末端位姿矩阵
    Td[0, 3] = Td[0, 3] + 0.01      # 目标位姿：让机械臂末端 X坐标 +0.01米

    q_sol = robot.sovler(q_ref, Td) # 求解的目标关节角

    print(f"q_ref: {q_ref}")      # 输入的参考关节角
    print(f"q_sol: {q_sol}")      # 求解的目标关节角  
    print(f"T_ref:\n{Td}")        # 目标末端位姿矩阵
    print(f"T_sol:\n{robot.fkine(q_sol)}")       # 验证：新关节角算出的末端位姿矩阵
