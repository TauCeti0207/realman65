...
LastEditTime: 2025-11-03 12:01:20
'''
import sys
...
class TestArmController(ArmController):
    ...
    def reset(self, start_state):
        if start_state.shape[0] == self.DoFs:
            debug_print(
                self.name, f"reset success, to start state \n {start_state}", self.INFO)
        else:
            debug_print(
                self.name, f"reset() input should be joint controll which dim is {self.DoFs}", "ERROR")
    ...
    def set_position(self, position):
        if position.shape[0] == 6:
            debug_print(
                self.name, f"using EULER set position to \n {position}", self.INFO)
        elif position.shape[0] == 7:
            debug_print(
                self.name, f"using QUATERNION set position to \n {position}", self.INFO)
        else:
            debug_print(
                self.name, f"set_position input size should be 6 -> EULER or 7 -> QUATERNION", "ERROR")

    def set_joint(self, joint):
        if joint.shape[0] != self.DoFs:
            debug_print(
                self.name, f"set_joint() input size should be {self.DoFs}", "ERROR")
        else:
    ...
    def set_gripper(self, gripper):
        if isinstance(gripper, (int, float, complex, np.ndarray)) and not isinstance(gripper, bool):
            if 1 > gripper > 0:
                ...
                debug_print(
                    self.name, f"gripper better be 0~1, but get number {gripper}", "WARNING")
        else:
            print(gripper.ndim)
            debug_print(
                self.name, f"gripper should be a number 0~1, but get type {type(gripper)}", "ERROR")
