...
from udp_client import extract_and_repack_finger_positions

from std_msgs.msg import String
import json
from scipy.spatial.transform import Rotation as R

class Quest3ToSMPLXConverter:
    def __init__(self):
        # --- 人体尺寸参数 (单位: 米) ---
        self.shoulder_width = 0.35
        self.torso_length = 0.55
        self.upper_arm_len = 0.28
        self.lower_arm_len = 0.25
        
        # 预存一份标准站立姿态的相对偏移量 (基于你提供的数据样本)
        # 我们将计算腿部相对于骨盆的偏移，手指相对于手腕的偏移
        self._init_template_offsets()

    def _init_template_offsets(self):
        """
        初始化模板：将提供的样本数据转换为相对于父节点的局部偏移。
        这样当骨盆或手腕移动时，腿和手指会跟着动。
        """
        # 1. 定义肢体层级关系 (Child -> Parent) 用于刚性跟随
        self.rigid_chains = {
            # 下半身跟随 Pelvis
            'left_hip': 'pelvis', 'right_hip': 'pelvis',
            'left_knee': 'pelvis', 'right_knee': 'pelvis', # 简化：直接跟随骨盆
            'left_ankle': 'pelvis', 'right_ankle': 'pelvis',
            'left_foot': 'pelvis', 'right_foot': 'pelvis',
            
            # 细节部位跟随 Head
            'jaw': 'head', 
            'left_eye_smplhf': 'head', 'right_eye_smplhf': 'head',
            
            # 手指跟随 Wrist (这里列出部分，代码里会处理所有以index/middle等开头的)
        }
        
        # 2. 你的样本数据 (作为基准参考)
        # 这是一个标准的 T-pose/A-pose 数据片段
        ref_pelvis_pos = np.array([-1.2823315 ,  0.14100393,  0.90017547])
        ref_pelvis_rot = R.from_quat([ 0.36568545,  0.35383203, -0.59844468, -0.61882228]) # xyzw
        
        ref_lwrist_pos = np.array([-1.51816381,  0.20155724,  1.28837831])
        ref_lwrist_rot = R.from_quat([-0.5509059 , -0.576333  , -0.00719739, -0.60356537])
        
        ref_rwrist_pos = np.array([-1.25332104,  0.69736358,  1.26324406])
        ref_rwrist_rot = R.from_quat([ 0.74874375,  0.39196542,  0.10295391, -0.524544  ])

        # 3. 计算相对偏移量 (Offset)
        # 格式: self.offsets['bone_name'] = (relative_pos_in_parent_frame, local_rotation)
        self.offsets = {}
        
        # 这是一个手动提取的简化版偏移库，基于你提供的数据
        # 为了代码简洁，这里直接硬编码腿部和手指相对于父级的"大概"位置
        # 实际运行时，我们把这些偏移应用到当前的 Pelvis/Wrist 上
        
        # 下半身相对位置 (基于样本数据计算出的相对 Pelvis 的差值)
        # 例如: L_Hip_Pos - Pelvis_Pos
        self.offsets['left_hip'] = np.array([-0.006, -0.062, -0.101])
        self.offsets['right_hip'] = np.array([ 0.043,  0.045, -0.112])
        self.offsets['left_knee'] = np.array([-0.142, -0.165, -0.459])
        self.offsets['right_knee'] = np.array([ 0.068,  0.244, -0.431])
        self.offsets['left_ankle'] = np.array([-0.005, -0.211, -0.853])
        self.offsets['right_ankle'] = np.array([ 0.117,  0.274, -0.857])
        self.offsets['left_foot'] = np.array([-0.164, -0.235, -0.868])
        self.offsets['right_foot'] = np.array([ 0.101,  0.435, -0.886])
        
        # 手指部分：这里为了省事，我们定义一个"标准手掌姿态"
        # 所有的手指骨骼名称
        self.finger_names = [
        ...
        # 默认四元数 (从你数据中取出的一个大概值)
        self.default_finger_quat = np.array([0.0, 0.0, 0.0, 1.0]) 

    def solve_two_bone_ik(self, root_pos, end_pos, pole_vector, upper_len, lower_len):
        """IK 解算器: 计算肘部位置"""
        root_to_end = end_pos - root_pos
        dist = np.linalg.norm(root_to_end)
        max_len = (upper_len + lower_len) * 0.99
        dist = min(dist, max_len)
        dist = max(dist, 0.01)
        
        direction = root_to_end / dist
        alpha = np.arccos(np.clip((upper_len**2 + dist**2 - lower_len**2) / (2 * upper_len * dist), -1.0, 1.0))
        
        elbow_base = root_pos + direction * (upper_len * np.cos(alpha))
        h = upper_len * np.sin(alpha)
        
        arm_plane_normal = np.cross(direction, pole_vector)
        if np.linalg.norm(arm_plane_normal) < 0.001:
            arm_plane_normal = np.array([0, 1, 0])
        arm_plane_normal /= np.linalg.norm(arm_plane_normal)
        
        elbow_dir = np.cross(arm_plane_normal, direction)
        elbow_dir /= np.linalg.norm(elbow_dir)
        
        elbow_pos = elbow_base + elbow_dir * h
        return elbow_pos

    def process(self, headset_data):
        """
        主处理函数
        """
        # 1. 获取输入数据 (ROS格式转Numpy)
        head_pos = np.array(headset_data.h_pos)
        head_quat = np.array(headset_data.h_quat) # [x, y, z, w]
        
        l_hand_pos = np.array(headset_data.l_pos)
        l_hand_quat = np.array(headset_data.l_quat)
        
        r_hand_pos = np.array(headset_data.r_pos)
        r_hand_quat = np.array(headset_data.r_quat)

        # 2. 估算骨盆 (Pelvis) - 上半身核心
        # 骨盆位置 = 头位置 - 躯干长 (简单垂直向下)
        # 为了让骨盆稍微自然点，我们让它跟随头的Yaw旋转，但保持直立
        head_rot = R.from_quat(head_quat)
        head_euler = head_rot.as_euler('xyz')
        
        # 构造躯干旋转 (只保留Yaw，去除低头/歪头的影响)
        body_yaw_rot = R.from_euler('xyz', [0, 0, head_euler[2]])
        pelvis_quat = body_yaw_rot.as_quat()
        
        # 骨盆位置
        pelvis_pos = head_pos - np.array([0, 0, self.torso_length])

        # 3. 估算肩膀 (Shoulder)
        # 肩膀相对于骨盆的位置偏移
        left_shoulder_offset = body_yaw_rot.apply(np.array([0, self.shoulder_width/2, self.torso_length * 0.85]))
        right_shoulder_offset = body_yaw_rot.apply(np.array([0, -self.shoulder_width/2, self.torso_length * 0.85]))
        
        l_shoulder_pos = pelvis_pos + left_shoulder_offset
        r_shoulder_pos = pelvis_pos + right_shoulder_offset

        # 4. 估算肘部 (Elbow) - IK
        # 肘部朝向向量 (向后下方)
        pole_vector = body_yaw_rot.apply(np.array([-0.5, 0, -1.0]))
        
        l_elbow_pos = self.solve_two_bone_ik(l_shoulder_pos, l_hand_pos, pole_vector, self.upper_arm_len, self.lower_arm_len)
        r_elbow_pos = self.solve_two_bone_ik(r_shoulder_pos, r_hand_pos, pole_vector, self.upper_arm_len, self.lower_arm_len)

        # 5. 组装数据字典
        # 注意：所有Key必须小写，Value必须是 (np.array, np.array)
        smplx_data = {}

        # --- A. 核心躯干 ---
        smplx_data['pelvis'] = (pelvis_pos, pelvis_quat)
        smplx_data['head'] = (head_pos, head_quat)
        smplx_data['neck'] = (head_pos - np.array([0,0,0.1]), head_quat)
        
        # 脊柱插值 (简单的线性插值)
        for i, name in enumerate(['spine1', 'spine2', 'spine3']):
            factor = (i + 1) / 4.0
            pos = pelvis_pos + (head_pos - pelvis_pos) * factor * 0.6
            smplx_data[name] = (pos, pelvis_quat)

        smplx_data['left_collar'] = (l_shoulder_pos + np.array([0, -0.05, 0]), pelvis_quat)
        smplx_data['right_collar'] = (r_shoulder_pos + np.array([0, 0.05, 0]), pelvis_quat)
        
        # --- B. 手臂 ---
        # 旋转暂时借用父级或EndEffector，GMR主要看位置
        smplx_data['left_shoulder'] = (l_shoulder_pos, pelvis_quat)
        smplx_data['left_elbow'] = (l_elbow_pos, l_hand_quat) 
        smplx_data['left_wrist'] = (l_hand_pos, l_hand_quat)
        
        smplx_data['right_shoulder'] = (r_shoulder_pos, pelvis_quat)
        smplx_data['right_elbow'] = (r_elbow_pos, r_hand_quat)
        smplx_data['right_wrist'] = (r_hand_pos, r_hand_quat)
        
        # --- C. 附属部位 (刚性跟随) ---
        # 下半身：相对于 Pelvis 保持静止
        for bone_name, offset in self.offsets.items():
            # 简单的向量加法 (因为 offset 是近似世界系差值，这里简单处理为跟随骨盆平移)
            # 为了更严谨，应该让 offset 跟随 pelvis 旋转，但对于"只控制上半身"，直接平移足够了
            # 这里的 offset 是基于你样本数据的"相对偏移"，我们加上当前的 pelvis_pos
            
            # 注意：这里的 offset 实际上是 样本_pos - 样本_pelvis
            # 所以 current_pos = current_pelvis + offset
            # (忽略了旋转带来的偏移变化，假设人站着不动转腰)
            if 'knee' in bone_name or 'ankle' in bone_name or 'foot' in bone_name or 'hip' in bone_name:
                # 让腿跟随骨盆的旋转 (Yaw)
                rotated_offset = body_yaw_rot.apply(offset)
                smplx_data[bone_name] = (pelvis_pos + rotated_offset, pelvis_quat)
        
        # 眼睛/下颚：跟随头部
        smplx_data['jaw'] = (head_pos, head_quat)
        smplx_data['left_eye_smplhf'] = (head_pos, head_quat)
        smplx_data['right_eye_smplhf'] = (head_pos, head_quat)

        # --- D. 手指 (填充数据以防报错) ---
        # 生成所有手指 Key，位置设为手腕位置，旋转设为手腕旋转
        # 这样手指会缩在手腕处，或者随手腕飞舞，但不会报错
        for finger in self.finger_names:
            l_key = f"left_{finger}"
            r_key = f"right_{finger}"
            
            # 左手手指跟随左手腕
            smplx_data[l_key] = (l_hand_pos, l_hand_quat)
            # 右手手指跟随右手腕
            smplx_data[r_key] = (r_hand_pos, r_hand_quat)
...
class UDPCameraServer(Node):
    def __init__(self): 
        ...
        self.ik_converter = Quest3ToSMPLXConverter()
        self.latest_smplx_data = None # 用来存最新的转换结果
        ...
        self.controller_pub = self.create_publisher(
            String, "quest/controller_state", 10)
    ...
    def start_server(self):
        try:
            ...
            self.threads = [
                ...
                threading.Thread(target=self.publish_finger_thread, daemon=True),
            ]
    ...
    def publish_finger_thread(self):
        """发布手指位置线程"""
        print("发布手指位置线程启动")
        freq = 72
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        server_addr = ("192.168.13.77", 9999)
    
        try:
            while True:
                # 直接使用最新的 self.last_headset_data
                if self.last_headset_data is not None:
                    try:
                        
                        # 将字典转换为JSON字符串再传入
                        msg = extract_and_repack_finger_positions(self.last_headset_data)
                        
                        if msg:
                            sock.sendto(msg.encode(), server_addr)
                            print('----------------------------------------------------------------')
                                # 可选：减少打印频率，避免日志过多
                                # print("send finger positions to 192.168.13.77")
                    except Exception as e:
                        print(f"发布手指位置错误: {e}")
                else:
                    print("警告: 没有可用的头显数据")
            ...
            print(f"publish_finger_thread 异常: {e}")
        finally:
            sock.close()
            print("发布手指位置线程关闭")
    ...
    def receive_data_thread(self):
        ...
        while self.running:
            try:
                data, addr = self.data_socket.recvfrom(16384)
                ...
                self.print_headset_data(headset_data)

                if self.headset_data is not None:
                    # 使用刚才写的工具类进行计算
                    self.latest_smplx_data = self.ik_converter.process(self.headset_data)
                    # print(self.latest_smplx_data)    
    ...
    def update_controller_state(self, data):
        ...
        controller_msg = {
            "timestamp": time.time(),
            "left": {
                "position": self.l_matrix[:3, 3].tolist(),
                "quaternion": R.from_matrix(self.l_matrix[:3, :3]).as_quat().tolist(),
                "button_one": bool(self.l_button_one),
                "thumbstick": [self.l_thumbstick_x, self.l_thumbstick_y],
                "index_trigger": self.l_index_trigger,
                "hand_trigger": self.l_hand_trigger,
                "button_two": bool(self.l_button_two),
                "button_thumbstick": bool(self.l_button_thumbstick),
            },
            "right": {
                "position": self.r_matrix[:3, 3].tolist(),
                "quaternion": R.from_matrix(self.r_matrix[:3, :3]).as_quat().tolist(),
                "thumbstick": [self.r_thumbstick_x, self.r_thumbstick_y],
                "index_trigger": self.r_index_trigger,
                "hand_trigger": self.r_hand_trigger,
                "button_one": bool(self.button_A),
                "button_two": bool(self.button_B),
                "button_thumbstick": bool(self.r_button_thumbstick),
            },
        }
        msg = String()
        msg.data = json.dumps(controller_msg)
