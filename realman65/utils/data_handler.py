...
def compute_rotate_matrix(pose):
    ...
    # R = Rotation.from_euler('XYZ', [roll, pitch, yaw]).as_matrix()  # 大写"XYZ" 固定轴外旋
    R = Rotation.from_euler('xyz', [roll, pitch, yaw]).as_matrix()    # 小写"xyz" 固定轴内旋
...
def compute_local_delta_pose(base_pose, target_pose):
    ...
    # 将pose转换为4x4齐次变换矩阵
    base_T = euler_to_matrix(base_pose)
    target_T = euler_to_matrix(target_pose)
    
    # 计算delta矩阵：T_delta = T_base^(-1) * T_target
    base_T_inv = np.linalg.inv(base_T)
    delta_T = base_T_inv @ target_T
    
    # 从delta矩阵提取xyz和rpy
    delta_pose = matrix_to_xyz_rpy(delta_T)
...
def apply_local_delta_pose(base_pose, delta_pose):
    ...
    # 将pose转换为4x4齐次变换矩阵
    base_T = euler_to_matrix(base_pose)
    delta_T = euler_to_matrix(delta_pose)
    
    # 计算target矩阵：T_target = T_base * T_delta
    target_T = base_T @ delta_T
    
    # 从target矩阵提取xyz和rpy
    target_pose = matrix_to_xyz_rpy(target_T)
