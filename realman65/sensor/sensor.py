...
LastEditTime: 2025-11-03 12:01:49
'''
import sys
sys.path.append("./")
...
class Sensor:
    ...
    def get(self):
        if self.collect_info is None:
            debug_print(
                {self.name}, f"collect_info is not set, if only collecting controller data, forget this warning", "WARNING")
            return None
        info = self.get_information()
        for collect_info in self.collect_info:
            if info[collect_info] is None:
                debug_print(f"{self.name}",
                            f"{collect_info} information is None", "ERROR")
