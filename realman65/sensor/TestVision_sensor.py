...
LastEditTime: 2025-11-03 12:01:31
'''
import time
import numpy as np
from realman65.utils.data_handler import debug_print
from realman65.sensor.vision_sensor import VisionSensor
import sys
sys.path.append("./")
...
class TestVisonSensor(VisionSensor):
    ...
    def set_up(self, input=None, is_depth=False):
        debug_print(
            self.name, f"setup success, is_depth={is_depth}", self.INFO)
    ...
    def get_image(self):
        ...
        if "color" in self.collect_info:
            image["color"] = np.random.randint(
                0, 256, size=(height, width, 3), dtype=np.uint8)

        if "depth" in self.collect_info:
            if not self.is_depth:
                debug_print(
                    self.name, f"should use set_up(is_depth=True) to enable collecting depth image", "ERROR")
                raise ValueError
            image["depth"] = np.random.randint(
                0, 256, size=(height, width, 3), dtype=np.uint8)
