...
class Realman65Interface:
    ...
    def reset(self) -> None:
        for arm_name in self.controllers.keys():
            joint_key = f"{arm_name.replace('_arm', '')}_start_position"
            start_pose = self.joint_config.get(joint_key)
            if start_pose is None:
                continue
            self.set_joint_angles(arm_name, start_pose)
            if self.device_config.get('gripper', False):
                try:
                    self.set_gripper(arm_name, 0)
                except Exception:
                    cprint(f"set {arm_name} gripper to 0 failed",color='red')
